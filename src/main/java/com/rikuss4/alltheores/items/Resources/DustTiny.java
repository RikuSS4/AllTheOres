...
public class DustTiny extends Item {
	private String name;
	private ATOOre ore;
	...
	public DustTiny(String name, ATOOre ore, int color, int renderType, int burnTime) {
		super();
		setName(name.toLowerCase() + "_" + "dustTiny");
		setUnlocalizedName(name.toLowerCase() + "_" + "dustTiny");
		setOre(ore);
		setColor(color);
		setBurnTime(burnTime);
		setOreDictName("dustTiny", name);
	...
	public String getItemStackDisplayName(ItemStack itemStack) {
		...
		return (StatCollector.translateToLocal("AllTheOres.dustTinySubstite")).replaceFirst("ORENAME", name);
	}
	...
	public void registerIcons(IIconRegister iconRegister) {
		if (iconRegister instanceof TextureMap) {
			...
			if (texture == null) {
				texture = new ItemTexture(this.getOre(), name, getColor(), getRenderType(), "dustTiny");
