...
public class BlockTexture extends TextureAtlasSprite {
	...
	public boolean hasCustomLoader(IResourceManager manager, ResourceLocation location) {
		ResourceLocation location1 = new ResourceLocation(location.getResourceDomain(),
		      String.format("%s/%s%s", "textures/" + (this.type == 1 ? "items" : "blocks"), location.getResourcePath(),
		            ".png"));
		try {
	...
	public boolean load(IResourceManager manager, ResourceLocation location) {
		...
		try {
			IResource iResourceBlock = manager
			      .getResource(new ResourceLocation(
			            Reference.MOD_ID.toLowerCase(),
			            "textures/" + getTypeName() + "/" + this.blockName + "_" + renderType + ".png"));

			base_image = ImageIO.read(iResourceBlock.getInputStream());
		} catch (IOException e) {
			base_image = new BufferedImage(16, 16, BufferedImage.TYPE_INT_RGB);
		}
		...
		try {
			IResource iResourceUnderlay = manager
			      .getResource(new ResourceLocation(
			            Reference.MOD_ID.toLowerCase(),
			            "textures/" + getTypeName() + "/" + this.blockName + renderType + ".png"));
		...
		BufferedImage overlay_image;
			try {
				IResource iResourceOverlay = manager
				      .getResource(new ResourceLocation(
				            Reference.MOD_ID.toLowerCase(),
				            "textures/" + getTypeName() + "/" + this.blockName + "_overlay_" + renderType + ".png"));
		...
		// load the texture
		this.loadSprite(block_image, animation,
