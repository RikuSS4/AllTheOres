...
public class ImageUtils {
	...
	public void doHSB(double t_hue, double t_sat, double t_bri,
			BufferedImage image) {
		hue = t_hue;
		saturation = t_sat;
		lightness = t_bri;
	...
	public static int makeColWithoutAlpha(int red, int green, int blue,
			int adjustPercent) {
	...
	public static int makeCol(int red, int green, int blue, int alpha,
		...
		color = (Math.min(alpha, 255) << 24) | (Math.min(red, 255) << 16)
				| (Math.min(green, 255) << 8) | Math.min(blue, 255);
		return color;
	}
	 ...
	 */
	public static BufferedImage colorize2(BufferedImage src, float r, float g,
			float b) {
		...
		for (int i = 0; i < newImage.getWidth(); i++) {
			for (int j = 0; j < newImage.getHeight(); j++) {
				...
				gx *= g;
				bx *= b;
				newImage.setRGB(i, j, (ax << 24) | (rx << 16) | (gx << 8)
						| (bx << 0));
			}
		}
		return newImage;
	...
	public static ResourceLocation getResource(String s2, int type) {
		...
		s1 = s1.toLowerCase();
		if (s2.startsWith("textures/items/")
				|| s2.startsWith("textures/blocks/")) {
			s2 = s2 + ".png";
		} else {
			s2 = "textures/" + ((type == 1) ? "items/" : "blocks/") + s2
					+ ".png";
		}
	...
	public static int getColor(Boolean avg, BufferedImage image,
			ResourceLocation textureLocation) {
		...
		try {
			...
			ImageIO.write(
					...
					"png",
					new File("TestImages\\"
							+ textureLocation.getResourcePath().replaceAll(
									"[^A-Za-z0-9()\\[\\]]", "_") + ".png"));
		} catch (IOException e) {
	...
	public static int getColor(Boolean avg, String... location) {
		if(location.length < 1 || location[0] == null) return -1;
	...
	public static int getColor(Boolean avg, ResourceLocation textureLocation) {
		...
		if (textureLocation != null) {
			try {
				bufferedimage = ImageIO.read(Minecraft.getMinecraft()
						.getResourceManager().getResource(textureLocation)
						.getInputStream());
			} catch (IOException e) {
		...
		return (bufferedimage != null) ? getColor(avg, bufferedimage,
				textureLocation) : -1;
	}
	...
	public static BufferedImage colorize(BufferedImage input_image, int color) {
		...
		BufferedImage output_image = new BufferedImage(w, h,
				BufferedImage.TYPE_4BYTE_ABGR);
		int[] image_data = new int[w * h];

		input_image.getRGB(0, 0, w, h, image_data, 0, w);
		...
		for (int y = 0; y < h; y += w) {
			...
			for (int i = 0; i < new_data.length; i++) {
				...
				if (getAlpha(col) != 0x00) {
					...
					new_data[i] = makeCol((getRed(color) * getRed(col)) / 0xff,
							(getGreen(color) * getGreen(col)) / 0xff,
							(getBlue(color) * getBlue(col)) / 0xff,
							getAlpha(col));
				}
			}

			// write the new image data to the output image buffer
			output_image.setRGB(0, y, w, h, new_data, 0, w);
		}
		return output_image;
	}
		...
		while (height-- != 0) {
			...
			while (width-- != 0) {
				...
				Color final_color = new Color(final_red_lookup[lum],
						final_green_lookup[lum], final_blue_lookup[lum],
						color.getAlpha());
	...
	public static BufferedImage changeBrightness(BufferedImage inImage,
	...
	public static int getBrightessColor(BufferedImage bi, int x0, int y0) {
		...
		for (int x = x0; x < x1; x++) {
			for (int y = y0; y < y1; y++) {
				...
				Color pixel = new Color(rgb);
				if (!isIncluded(gray, rgb, 50) && !isIncluded(white, rgb, 50)) {
					if(pixel.getRed() > brightessRed ||
							pixel.getGreen() > brightessGreen ||
									pixel.getBlue() > brightessBlue
							) {
